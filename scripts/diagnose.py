...
async def dump_page(ctx, name: str, url: str, debug_dir: Path, console_lines: list, failed_reqs: list):
    """打开一个 URL，等渲染，保存 HTML + 截图，记录 console / 失败请求"""
    ...
    try:
        async with pool.get_context() as ctx:
            ...
            await dump_page(
                ...
                debug_dir, console_lines, failed_reqs,
            )
            await dump_page(
                ctx, "detail",
